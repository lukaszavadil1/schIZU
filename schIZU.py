...
for i in range(len(path) - 1):
    ...
    reward = 0 if abs(int(states_rating[(next_state - 1) // 5][(next_state - 1) % 5])) != 1 else int(states_rating[(next_state - 1) // 5][(next_state - 1) % 5])
...
for row in states_rating:
    print(" ".join([f"{rating:.3f}" if abs(rating) != 1 else f"rew={int(rating)}" for rating in row]))
